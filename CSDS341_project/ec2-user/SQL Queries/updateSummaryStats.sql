use CSDS341_Project;
DROP PROCEDURE IF EXISTS updateSummaryStats;
DELIMITER // 
Create Procedure updateSummaryStats (IN param_company VARCHAR(20), IN param_date varchar(20), IN param_quarter INT, IN param_year INT)
BEGIN
...
SELECT max(T.split_adjusted_price) as max_52_week FROM
	(SELECT date, price / PriceAdjustment as split_adjusted_price FROM Securities 
    WHERE ticker = param_company AND date BETWEEN DATE_ADD(param_date, IN
